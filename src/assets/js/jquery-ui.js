...
;(function (factory) {
  ...
  $.widget('ui.resizable', $.ui.mouse, {
    ...
    _mouseStop: function (event) {
      ...
      if (this._helper) {
        ...
        top =
          parseFloat(that.element.css('top')) + (that.position.top - that.originalPosition.top) ||
          null
  ...
  $.ui.plugin.add('resizable', 'containment', {
    ...
    resize: function (event) {
      ...
      hoset = Math.abs(
        that.sizeDiff.height + (that._helper ? that.offset.top - cop.top : that.offset.top - co.top)
      )
        ...
        .resizable({
          ...
          stop: function (event, ui) {
            ...
            options.position = {
              my: 'left top',
              at:
                'left' + (left >= 0 ? '+' : '') + left + ' ' + 'top' + (top >= 0 ? '+' : '') + top,
              of: that.window
            }
    ...
    $.ui.formResetMixin,
    {
      ...
      _documentClick: {
        mousedown: function (event) {
          ...
          if (
            !$(event.target).closest('.ui-selectmenu-menu, #' + $.escapeSelector(this.ids.button))
              .length
          ) {
            this.close(event)
          }
